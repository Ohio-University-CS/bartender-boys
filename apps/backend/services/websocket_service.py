...
class WebSocketConnectionManager:
        ...
        try:
            ...
            async with self._lock:
                if client_id in self.active_connections:
                    ...
                    try:
                        await old_ws.close(
                            code=1001, reason="Superseded by new connection"
                        )
                    except Exception:
            ...
            logger.error(
                f"Failed to accept WebSocket for {client_id}: {e}", exc_info=True
            )
    ...
    async def handle_connection(
        ...
        if not registered_ws:
            logger.error(
                f"WebSocket not found in active_connections after accept for {client_id}"
            )
            return
        ...
        if on_connect:
            try:
                ...
                logger.error(
                    f"Error in on_connect callback for {client_id}: {e}", exc_info=True
                )
        ...
        try:
                ...
                try:
                    ...
                    if on_message:
                        try:
                            ...
                            logger.error(
                                f"Error in on_message callback for {client_id}: {e}",
                                exc_info=True,
                            )
            ...
            if on_disconnect:
                try:
                    ...
                    logger.error(
                        f"Error in on_disconnect callback for {client_id}: {e}",
                        exc_info=True,
                    )
