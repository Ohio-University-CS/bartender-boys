...
from data.drinks import create_drink, get_drinks_paginated, get_drink_by_id as get_drink_by_id_db
from .models import (
...
async def get_drinks(
    ...
    limit: int = Query(20, ge=1, le=100, description="Maximum number of drinks to return"),
    ...
    try:
        ...
        for doc in drinks_docs:
            ...
            if not all(key in doc for key in ["name", "category", "ingredients", "instructions", "difficulty", "prepTime"]):
                logger.warning("Skipping drink document with missing required fields: %s", doc.get("_id", "unknown"))
                continue
...
async def generate_drink(request: GenerateDrinkRequest) -> GenerateDrinkResponse:
    ...
    try:
        ...
        logger.info("Drink created and saved successfully: %s (ID: %s)", saved_drink.name, saved_drink.id)
