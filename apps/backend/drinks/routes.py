...
async def generate_drink(
    ...
    try:
        ...
        if ratios is None:
            logger.warning(f"Ratios not provided, auto-generating for {len(request.ingredients)} ingredients")
            ratios = generate_ratios(request.ingredients)
            logger.info(f"Auto-generated ratios: {ratios}")
        else:
            # Validate ratios sum to 100
            if sum(ratios) != 100:
                logger.warning(f"Ratios sum to {sum(ratios)} instead of 100, normalizing")
