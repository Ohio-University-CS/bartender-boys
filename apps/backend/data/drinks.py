...
async def create_drink(
    ...
    logger.info("Created new drink record: %s (ID: %s) for user: %s", drink.name, drink_doc["_id"], user_id)
    return drink_doc
...
async def get_drinks_paginated(
    ...
    cursor = db["drinks"].find(query_filter).sort("created_at", -1).skip(skip).limit(limit)
    drinks = await cursor.to_list(length=limit)
    
    logger.info("Retrieved %d drinks (skip=%d, limit=%d, total=%d)", len(drinks), skip, limit, total_count)
