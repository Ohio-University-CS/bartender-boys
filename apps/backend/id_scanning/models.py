...
class IDScanResponse(BaseModel):
    ...
    state: Optional[str] = Field(None, description="State or jurisdiction that issued the ID")
    ...
    drivers_license_number: Optional[str] = Field(None, description="Driver's license number starting with 'VH'")
    is_valid: Optional[bool] = Field(None, description="Whether this appears to be a legitimate ID")
    error: Optional[str] = Field(None, description="Error message if processing failed")
    raw_response: Optional[str] = Field(None, description="Raw OpenAI response for debugging")
